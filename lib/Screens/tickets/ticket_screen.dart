...
class _TicketScreenState extends State<TicketScreen> {
  ...
  // Agrega producto y recalcula (provider como fuente de verdad)
  void _onItemSelected(Product p) {
    final prov = context.read<FacturasProvider>();
    final inv = prov.getInvoiceByIndex(widget.index);
    inv.detail ??= <Product>[];
    inv.detail!.add(p);
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      "Saldo:${NumberFormat("###,000", "en_US").format(factura.saldo)}",
                      style: const TextStyle(
                        color: kNewtextPri,
                        fontSize: 20,
                        fontWeight: FontWeight.w600,
  ...
  Widget _infoTicketSection(Invoice factura) {
    return Container(
      ...
      decoration: const BoxDecoration(color: kNewborder),
      child: Column(
        children: [
          const Text(
            'Información Ticket',
