...
class POSDetailPage extends StatelessWidget {
  ...
  const POSDetailPage(
      {super.key, required this.face, required this.dispatchId});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
            'POS ${face.pos} · ${face.pumpName ?? "Surtidor ${face.pumpId}"} · Cara ${face.face}'),
        centerTitle: true,
      ),
      body: ListView.separated(
        ...
        itemBuilder: (_, i) {
          ...
          return _HoseCard(
            hose: h,
            onSelect: () {
              final prov = context.read<AltDespachosProvider>();
              final ctrl = prov.getById(dispatchId);
              if (ctrl == null) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                      content: Text(
                          'No se encontró el DispatchControl del flujo actual')),
                );
                return;
              }
                ...
                child: Column(
                    ...
                    children: [
                      ...
                      Row(children: [
                        _PricePill(
                            label: 'Cash',
                            value: hose.priceCash,
                            decimals: hose.priceDecimals),
                        const SizedBox(width: 6),
                        _PricePill(
                            label: 'Credit',
                            value: hose.priceCredit,
                            decimals: hose.priceDecimals),
                        const SizedBox(width: 6),
                        _PricePill(
                            label: 'Debit',
                            value: hose.priceDebit,
                            decimals: hose.priceDecimals),
                      ]),
                    ]),
              ),
...
class _PricePill extends StatelessWidget {
  ...
  final int decimals;
  const _PricePill(
      {required this.label, required this.value, required this.decimals});
  @override
  Widget build(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
    return Container(
