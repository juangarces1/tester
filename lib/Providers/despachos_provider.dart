...
class DespachosProvider extends ChangeNotifier {
  ...
  final Map<DispatchControl, VoidCallback> _stageWatchers =
      {}; // <- track de listeners por control
  ...
  String getHoseDisplayLabel(String hoseKey) {
    ...
    switch (st) {
      case HoseStatus.available:
        return 'Disponible';
      case HoseStatus.authorized:
        return 'Autorizada';
      case HoseStatus.busy:
        return 'Ocupada';
      case HoseStatus.fueling:
        return 'Despachando';
      case HoseStatus.unpaid:
        return 'Pendiente de pago';
      case HoseStatus.stopped:
        return 'Detenida';
      case HoseStatus.finished:
        return 'Finalizado'; // compat
      case HoseStatus.unknown:
      default:
        return _beautifyRaw(raw) ?? 'Estado desconocido';
    }
  }
    ...
    try {
      ...
      for (final disp in allDispensers) {
        for (final hose in disp.hoses) {
          ...
          if (_hoseStatuses[key] != parsed) {
            _hoseStatuses[key] = parsed;
            changed = true;
          }
        }
      }
      ...
      for (final key in _watchedHoses) {
        if (!seen.contains(key)) {
          if (_hoseStatuses[key] != HoseStatus.unknown) {
            _hoseStatuses[key] = HoseStatus.unknown;
            changed = true;
          }
          if (_hoseRaw.containsKey(key)) {
            _hoseRaw.remove(key);
            changed = true;
          }
        }
  ...
  HoseStatus _parseStatus(String raw) {
    ...
    switch (token) {
      case 'available':
        return HoseStatus.available;
      case 'authorized':
        return HoseStatus.authorized;
      case 'fueling':
        return HoseStatus.fueling;
      case 'unpaid':
        return HoseStatus.unpaid;
      case 'stopped':
        return HoseStatus.stopped;
      case 'busy':
        return HoseStatus.busy;
      case 'finished':
        return HoseStatus.finished; // compat
      default:
