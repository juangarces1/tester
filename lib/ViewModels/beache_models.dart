...
class DispensersDashboard extends StatefulWidget {
  ...
  const DispensersDashboard({Key? key, required this.isActive})
      : super(key: key);
...
/* ───────── ciclo de vida ───────── */
  ...
  ///  Los estados llegan por SignalR (MapProvider). Aquí solo construimos
  ///  la topología visual con hoseStatus = null (se actualiza en tiempo real).
  /// ---------------------------------------------------------------------------
  Future<List<DispensadorVisual>> _loadData() async {
    // Sin estados REST: los estados vienen por SignalR vía MapProvider.
    // Por ahora retornamos lista vacía hasta integrar la topología del nuevo API.
    debugPrint(
        '[DispensersDashboard] _loadData: topología pendiente de nuevo API');
    return [];
  }
...
class _DispensadorCardVisual extends StatefulWidget {
  ...
  const _DispensadorCardVisual({Key? key, required this.datos})
      : super(key: key);
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            Text(
              widget.datos.name.isNotEmpty
                  ? widget.datos.name
                  : 'Dispensador ${widget.datos.id}',
            ...
            Row(
              ...
              children: [
                Expanded(
                    child: _caraColumn(
                        widget.datos.caraA, widget.datos.caraA?.letra ?? '1')),
                const SizedBox(width: 14),
                Expanded(
                    child: _caraColumn(
                        widget.datos.caraB, widget.datos.caraB?.letra ?? '2')),
              ],
            ),
          ],
        ),
...
// NUEVO: Estado en su propia fila (píldora linda)
  ...
  String _getCaraStatus(List<MangueraVisual> hoses) {
    ...
    for (final h in hoses) {
      ...
      if (s == 'authorized')
        anyAuthorized = true; // ← FIX: antes estaba al revés
    }
...
class _PreDispenseDialogState extends State<PreDispenseDialog> {
  ...
  Future<void> _callPreDispense() async {
    ...
    final nozzleCode = widget.mv.nozzleNumber.toString().padLeft(2, '0');
    final ok = await ConsoleApiHelper.presetByAmount(
      nozzleCode: nozzleCode,
      amount: amount,
      tagId: 'B32809EE018B2811', // TODO: reemplazar con tag real del frentista
  ...
  Future<void> _callPostDispense() async {
    setState(() => _loading = true);
    final nozzleCode = widget.mv.nozzleNumber.toString().padLeft(2, '0');
    final ok = await ConsoleApiHelper.presetTankFull(
      nozzleCode: nozzleCode,
      tagId: 'B32809EE018B2811', // TODO: reemplazar con tag real del frentista
    );
    if (!mounted) return;
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content:
            Text(ok ? 'Tanque lleno autorizado 🟢' : 'Error al autorizar ❌'),
        backgroundColor: ok ? Colors.green : Colors.red,
      ),
    );
  }
    ...
    return Dialog(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
                'Despachar ${_fuelFromDescription(widget.mv.descripcion)}-M${widget.mv.nozzleNumber}',
                style: const TextStyle(
                    fontSize: 18,
                    fontWeight: FontWeight.bold,
                    color: Colors.white)),
                  ...
                  Expanded(
                    child: ElevatedButton(
                      ...
                      style: ElevatedButton.styleFrom(
                          backgroundColor:
                              const Color.fromARGB(255, 53, 123, 55)),
                      child: const Text(
                        'TANQUE LLENO',
                        style: TextStyle(
...
class DispatchesBottomSheet extends StatefulWidget {
  ...
  const DispatchesBottomSheet(
      {super.key, required this.nozzle, required this.fuelname});
              ...
              itemBuilder: (_, i) {
                if (i < _items.length) {
                  ...
                  return _DispatchCard(
                      d: d, fuelname: widget.fuelname); // tu card de UI
                }
